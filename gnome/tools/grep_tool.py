...
class GrepTool(Tool):
    ...
    def description(self) -> str:
        return (
            "ИНСТРУМЕНТ ДЛЯ ПОИСКА В КОДЕ ПРОЕКТА. Это твой собственный tool — "
            "вызывай его когда юзер просит найти строку, переменную, функцию в "
            "коде или конфигах. Ищет по содержимому файлов в репозитории "
            "kontent-zavod-wb (где ты живёшь). По умолчанию по *.py/*.md/*.yaml. "
            "Возвращает до 30 совпадений в формате 'path:line: match'. "
            "Например для поиска переменной KIE_API_KEY вызови с pattern='KIE_API_KEY'."
        )
