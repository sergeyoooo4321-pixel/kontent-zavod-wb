...
async def process_product_images(
    ...
    """Диспатчер по settings.PIPELINE_MODE.

    legacy — 4 параллельные edit-генерации от src (refactor_plan.md §1).
    hybrid — rembg + AI-фон + PIL composite + Playwright plashki (§4).
    """
    if settings.PIPELINE_MODE == "hybrid":
        await process_product_images_hybrid(state, batch_id, chat_id, deps, sem)
    else:
        await process_product_images_legacy(state, batch_id, chat_id, deps, sem)


async def process_product_images_legacy(
    state: ProductState,
    batch_id: str,
    chat_id: int,
    deps: Deps,
...
# ─── Гибридный pipeline (refactor_plan.md §4) ────────────────────


def _build_plashki_context(brief: dict, mode: str) -> dict[str, Any]:
    """Собирает Jinja2-контекст для card_plashki.html.j2 под конкретный mode."""
...
async def process_product_images_hybrid(
    state: ProductState,
    batch_id: str,
    chat_id: int,
    deps: Deps,
    ...
    """Гибрид: rembg → AI-фон без товара → PIL composite × N → Playwright plashki overlay → S3.

    Преимущество: товар идентичен на main/pack2/pack3 буквально попиксельно (тот же файл).
    Текст плашек рендерится через HTML/CSS — нулевой брак на типографике.
    """
    async with sem:
        # 1. src → S3 + raw bytes для rembg
        try:
            raw = await deps.tg.get_file_bytes(state.tg_file_id)
            src_key = S3Client.build_key(batch_id, state.sku, "src")
            state.src_url = await deps.s3.put_public(src_key, raw, "image/jpeg")
        ...
        try:
            await deps.tg.send(
                chat_id,
                f"📥 `{state.sku}`: фото в S3 → вырезаю товар и анализирую сцену…",
                parse_mode="Markdown",
            )
        except Exception:
            pass

        # 2. rembg → product.png (lazy импорт, U2Net тяжёлый)
        product_png: bytes | None = None
        try:
            from .bg_remove import remove_bg
            product_png = remove_bg(raw)
            await deps.s3.put_public(
                S3Client.build_key(batch_id, state.sku, "product"),
                product_png,
                "image/png",
            )
        except Exception as e:
            state.errors.append(f"bg_remove: {e}")
            logger.exception("bg_remove %s: %s", state.sku, e)
            return
        ...
        try:
            ...
            design = brief.get("design") or {}
            logger.info("brief %s: scene=%s", state.sku, (design.get("scene") or "")[:60])
        except Exception as e:
            logger.warning("brief %s failed, using defaults: %s", state.sku, e)
            brief = {}

        # 4. AI генерит 4 фона параллельно — text-to-image, без input_urls
        modes = ["main", "pack2", "pack3", "extra"]

        async def gen_bg(mode: str) -> tuple[str, bytes | None, str | None]:
            try:
                prompt = compile_bg_only_prompt(brief, mode)
                kie_url = await deps.kie.generate_image_with_retry(
                    prompt=prompt,
                ...
                bg_bytes = await deps.s3.fetch(kie_url)
                return mode, bg_bytes, None
            except (KieAIError, KieAITimeout, S3Error) as e:
                return mode, None, str(e)
            except Exception as e:
                logger.exception("gen_bg %s/%s: %s", state.sku, mode, e)
                return mode, None, f"unexpected: {e}"

        bg_results_raw = await asyncio.gather(
            *[gen_bg(m) for m in modes],
            return_exceptions=True,
        )
        bgs: dict[str, bytes] = {}
        failed_bg: list[str] = []
        for res in bg_results_raw:
            if isinstance(res, BaseException):
                logger.error("hybrid gen_bg gather got exception: %s", res)
                continue
            mode, bg_bytes, err = res
            if bg_bytes:
                bgs[mode] = bg_bytes
            else:
                state.errors.append(f"bg {mode}: {err}")
                failed_bg.append(mode)

        if failed_bg:
            try:
                await deps.tg.send(
                    chat_id,
                    f"⚠️ `{state.sku}`: фон не сгенерировался — {', '.join(failed_bg)}. Остальные ОК.",
                    parse_mode="Markdown",
                )
            except Exception:
                pass
        ...
        # 5. composite + plashki overlay + S3
        from .composite import composite_card
        from .plashki import overlay_plashki, render_html_to_png
